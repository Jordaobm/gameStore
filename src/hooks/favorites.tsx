...
const FavoritesContextProvider: React.FC = ({ children }) => {
  ...
  const addProductInFavorites = useCallback(
    (product: Product) => {
      const alreadyExistInFavorite = favorites.find(
  ...
  const removeProductTheFavorites = useCallback(
    (product: Product) => {
  ...
  return (
    <FavoritesContext.Provider
      value={{ addProductInFavorites, removeProductTheFavorites, favorites }}
    >
      {children}
    </FavoritesContext.Provider>
  );
